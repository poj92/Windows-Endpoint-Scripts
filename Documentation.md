...
|---------|-----------|-------|
| SYSTEM | Detection, background tasks, privileged operations | No user interaction (unless InteractiveToken used) |
| Administrator User | Interactive scripts, user notifications | Full access to user context and resources |
| Interactive User | Browser/UI scripts requiring desktop access | Requires Datto InteractiveToken or scheduled task |

### Logging Standards

All scripts follow consistent logging patterns:
```powershell
$LogPath = "$env:ProgramData\[Company|Tool]\[ScriptName].log"
...
**Datto RMM Variable Binding**:
- String parameters: `[string]$ParamName = "$env:VarName"` 
- Boolean parameters: Use helper function like `Get-EnvBool`
- Component variables defined in Datto UI matched to parameter names

**Exit Codes**:
- Standard: 0 (success), 1 (needs action), 2-3 (warnings/errors)
- RMM parses exit codes for detection/remediation workflows

**Log Locations**:
- Datto: `C:\ProgramData\Datto\Logs\`
- Nexus: `C:\ProgramData\NexusOpenSystems\`
- Tool-specific: `C:\ProgramData\[ToolName]\`

### Common Parameters

Most scripts share these optional parameters:
- `-ReportOnly` - Execute in report mode without changes
- `-LogPath` - Custom log file location
- `-Force` - Skip confirmations and proceed
- `-Cleanup` - Clean residual files/configuration
...
1. **Header Documentation**: Include clear purpose, execution context, and key features
2. **RMM Parameters**: Document all Datto component variables with types and defaults
3. **Exit Codes**: Specify all possible exit codes for RMM detection/remediation
4. **Logging**: Use consistent logging format with timestamps and levels
5. **Update Documentation**: Add entry to this file following the existing format

### Updates to Existing Scripts

When updating existing scripts:

1. Test exit codes thoroughly (RMM depends on these)
2. Update all RMM component variable documentation
3. Document behavior changes in this guide
4. Maintain backward compatibility when possible
5. Update log locations if changed

### Script Testing Checklist

- [ ] Test in Detect mode (if applicable)
- [ ] Test in ReportOnly mode
- [ ] Verify exit codes match documented values
- [ ] Check log file creation and content
- [ ] Validate RMM variable parsing
...
- **PowerShell Version**: 5.1+ required for most scripts
- **Execution Policy**: Most require RemoteSigned minimum
- **Administrator Rights**: Required for system modifications
- **Network Access**: Some scripts require internet connectivity for downloads/updates

For issues or feature requests, refer to repository guidelines.
