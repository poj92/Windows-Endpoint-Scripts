...
function Classify-DotNetArpFamily([string]$DisplayName) {
  ...
  # Hosting Bundle should be treated with ASP.NET
  if ($DisplayName -match '(?i)Hosting Bundle')          { return 'AspNet' }
  if ($DisplayName -match '(?i)ASP\.NET Core')           { return 'AspNet' }

  # Host / HostFxr should be treated with Runtime
  if ($DisplayName -match '(?i)Host FX Resolver|HostFxr') { return 'Runtime' }
  if ($DisplayName -match '(?i)(^|\s)\.NET Host($|\s|\()|(^|\s)Microsoft \.NET Host($|\s|\()') { return 'Runtime' }
  if ($DisplayName -match '(?i)\.NET Runtime|Microsoft \.NET Runtime') { return 'Runtime' }

  if ($DisplayName -match '(?i)\.NET SDK|(^|\s)SDK($|\s|\()')       { return 'SDK' }
...
function Remove-FolderIfBelowMin([string]$Path, [Version]$Min) {
  ...
  Get-ChildItem -Path $Path -Directory -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    if ($v -and $v -lt $Min) {
      if ($PSCmdlet.ShouldProcess($folder.FullName, "Delete below-min folder")) {
        try {
          Remove-Item -Path $folder.FullName -Recurse -Force -ErrorAction Stop
          Write-Log "Deleted folder below min: $($folder.FullName)"
        }
        catch {
          $hadFailures = $true
...
try {
  ...
  Write-Log ("Below-min (ARP, monitored .NET families): {0} item(s)" -f $belowArp.Count)
  ...
  if ($belowArp2.Count -gt 0) {
    ...
    foreach ($g in $groups) {
      ...
      $exitCode = Uninstall-ArpEntry $item.Entry -Force:$ForceUninstallTool
      if ($exitCode -eq 3010) {
        $rebootRequired = $true
      }
    }
  ...
  if ($folderCleanupFailures) {
    Write-Log "One or more below-min folders could not be deleted because files were in use or access was denied. The script does not force a restart; rerun after applications close or after the next reboot."
  }

  if ($rebootRequired) {
    Write-Log "At least one uninstall requires a reboot to finish cleanup. Restart was not forced by this script."
  }
