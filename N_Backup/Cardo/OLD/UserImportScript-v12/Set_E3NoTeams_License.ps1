...
foreach($User in $Users){
    ...
    Write-Host "Error: The E3Only field for $($User.'User principal name') is empty, please check the csv file." -ForegroundColor Yellow
    }
    ...
    elseif($User.'E3Only' -eq 'Yes'){
        ...
        Set-ADUser $SamAccountName -Replace @{'extensionAttribute3' = $User.'E3Only'}
        Write-Host "Success: An E3 (no Teams) license will be assigned to $($User.'User principal name')" -ForegroundColor Green
        }
    ...
    elseif($User.'E3Only' -eq 'No'){
        ...
        Write-Host "Success: An E3 (no Teams) license will be removed from $($User.'User principal name')" -ForegroundColor Yellow
        }
    else{
    Write-Host "The E3Only field for $($User.'User principal name') is not a valid entry, no changes were made to this user's account" -ForegroundColor Red
        }
}
