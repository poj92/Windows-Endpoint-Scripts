...
foreach($User in $Users){
    ...
    Write-Host "Error: The Telephone number field for $($User.'User principal name') is empty, please check the template or csv file." -ForegroundColor Yellow
    break
    }
    
    # Set Address
    else{
        Set-ADUser $SamAccountName -OfficePhone $User.'Telephone number'
        Write-Host "Success: The Telephone Number for $($User.'User principal name') has been set to $($User.'Telephone number')" -ForegroundColor Green
        }
}
