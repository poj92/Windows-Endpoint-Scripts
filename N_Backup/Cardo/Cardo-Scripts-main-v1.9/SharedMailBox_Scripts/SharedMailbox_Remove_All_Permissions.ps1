<#
.SYNOPSIS
    Removes ALL permission types for users from shared mailboxes based on two CSV files.
...
.DESCRIPTION
    ...
    checks if each user has any permissions on each mailbox, and removes ALL permission types
    (FullAccess, ReadPermission, ChangePermission, ChangeOwner, etc.).
...
.EXAMPLE
    .\SharedMailbox_Remove_All_Permissions.ps1 -UsersCsvPath "C:\users.csv" -MailboxesCsvPath "C:\mailboxes.csv"
...
# Logging setup
$LogFile = "SharedMailbox_AllPermissions_Removal_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
function Write-Log {
...
foreach ($User in $Users) {
    ...
    foreach ($Mailbox in $Mailboxes) {
        ...
        try {
            ...
            # Check and remove mailbox permissions (FullAccess, ReadPermission, ChangePermission, ChangeOwner)
            $CurrentPermissions = Get-MailboxPermission -Identity $SharedMailboxEmail -User $UserEmail -ErrorAction SilentlyContinue

            if ($CurrentPermissions) {
                # Iterate through each permission and remove all AccessRights
                foreach ($Permission in $CurrentPermissions) {
                    $AccessRights = $Permission.AccessRights
                    
                    if ($AccessRights -and $AccessRights.Count -gt 0) {
                        Write-Log "Found mailbox permissions for $UserEmail on $SharedMailboxEmail`: $($AccessRights -join ', '). Removing..." "WARNING"
                        
                        # Remove each access right
                        foreach ($Right in $AccessRights) {
                            try {
                                Remove-MailboxPermission -Identity $SharedMailboxEmail -User $UserEmail -AccessRights $Right -InheritanceType All -Confirm:$false -ErrorAction Stop
                                Write-Log "  Removed $Right permission" "INFO"
                                $PermissionsRemoved++
                            }
                            catch {
                                Write-Log "  Failed to remove $Right permission: $_" "ERROR"
                            }
                        }
                    }
                }
            ...
            # Check and remove Send As permissions
            $SendAsPermissions = Get-RecipientPermission -Identity $SharedMailboxEmail -Trustee $UserEmail -ErrorAction SilentlyContinue | Where-Object { $_.AccessRights -contains "SendAs" }
            
            if ($SendAsPermissions) {
                Write-Log "Found Send As permission for $UserEmail on $SharedMailboxEmail. Removing..." "WARNING"
                try {
                    Remove-RecipientPermission -Identity $SharedMailboxEmail -Trustee $UserEmail -AccessRights SendAs -Confirm:$false -ErrorAction Stop
                    Write-Log "  Removed SendAs permission" "INFO"
                    $PermissionsRemoved++
                }
                catch {
                    Write-Log "  Failed to remove SendAs permission: $_" "ERROR"
                }
            }
            
            # Check and remove Send On Behalf permissions
            $Mailbox = Get-Mailbox -Identity $SharedMailboxEmail -ErrorAction SilentlyContinue
            if ($Mailbox.GrantSendOnBehalfTo -contains $UserEmail) {
                Write-Log "Found Send On Behalf permission for $UserEmail on $SharedMailboxEmail. Removing..." "WARNING"
                try {
                    Set-Mailbox -Identity $SharedMailboxEmail -GrantSendOnBehalfTo @{Remove=$UserEmail} -ErrorAction Stop
                    Write-Log "  Removed SendOnBehalf permission" "INFO"
                    $PermissionsRemoved++
                }
                catch {
                    Write-Log "  Failed to remove SendOnBehalf permission: $_" "ERROR"
                }
            }
            
            if ($PermissionsRemoved -gt 0) {
                Write-Log "Successfully removed $PermissionsRemoved permission(s) for $UserEmail from $SharedMailboxEmail" "SUCCESS"
                $RemovedCount++
            }
            else {
