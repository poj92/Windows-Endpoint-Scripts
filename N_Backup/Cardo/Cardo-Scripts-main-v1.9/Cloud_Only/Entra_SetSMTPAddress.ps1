<#
.DESCRIPTION
    This script updates the SMTP address of users in Entra ID by performing the following actions:
    - Imports user details from a CSV file, automatically uses "UserImport.csv" in the script directory if not specified
    - Updates the user's SMTP address based on the "E-mail" field in the CSV
...
.EXAMPLE
    ...
    .\Entra_SetSMTPAddress.ps1 -CsvPath "C:\Path\To\UserImport.csv"
.NOTES
...
try {
    ...
    $logFile = Join-Path $scriptDir "Entra_SetSMTPAddress_$timestamp.log"
    function Write-Log {
    ...
    foreach ($user in $users) {
        try {
            ...
            $newSmtpAddress = $user.'E-mail'
            if (-not $userPrincipalName) {
                Write-Log "Skipping entry with missing User Principal Name." -Color Yellow
                continue
            }
            if (-not $newSmtpAddress) {
                Write-Log "Skipping user $userPrincipalName due to missing E-mail field." -Color Yellow
                continue
            }
            # Update user's SMTP address
            # Update user's default SMTP address (Mail property)
            Write-Log ("Updating default SMTP address for user {0} to {1}..." -f $userPrincipalName, $newSmtpAddress)
            Update-MgUser -UserId $userPrincipalName -Mail $newSmtpAddress -ErrorAction Stop
            Write-Log ("Successfully updated default SMTP address for user {0}." -f $userPrincipalName) -Color Green
        }   
        catch {
    ...
    Write-Log "SMTP address update process completed." -Color Green
}
catch {
